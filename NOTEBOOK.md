...
validate, round trip 187/187, 0 errors, 0 ascription warnings.

**`.bast` are regenerated by the staged rc.9-54 and committed** — 187 from
the round-trip script plus the 2 pattern examples, which that script excludes
(BACKLOG #2) and which had to be bastified by hand after their source
changed. Watch for that pair: a `patterns/` source edit does **not** get its
`.bast` refreshed by any gate.

**Open, needs your call:** `../riddl` has a modified
`RiddlModelsRoundTripTest.scala`, and a riddl-side run of it rewrote these
`.bast` on 2026-08-05 from riddl's *working-tree* build — bytes that differ
from the staged rc.9-54 output. That rewrite was reverted here during
diagnosis. Once `../bin/riddlc` is restaged from the newer build, regenerate
